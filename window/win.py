...
class LeftTabWidget(QWidget):
    ...
    def initUi(self):
        ...
        item = QListWidgetItem(QIcon('Data/0%d.ico' % randint(1, 8)), '达人库', self.listWidget)
        item.setSizeHint(QSize(16777215, 60))
        item.setTextAlignment(Qt.AlignCenter)

        label = QLabel('达人库页面', self)
        label.setAlignment(Qt.AlignCenter)
        # 设置label的背景颜色(这里随机)
        # 这里加了一个margin边距(方便区分QStackedWidget和QLabel的颜色)
        label.setStyleSheet('background: rgb(%d, %d, %d);margin: 50px;' % (
        ...
        item = QListWidgetItem(QIcon('Data/0%d.ico' % randint(1, 8)), '达人收集', self.listWidget)
        item.setSizeHint(QSize(16777215, 60))
        item.setTextAlignment(Qt.AlignCenter)

        label = QLabel('达人收集页面', self)
        label.setAlignment(Qt.AlignCenter)
        # 设置label的背景颜色(这里随机)
        # 这里加了一个margin边距(方便区分QStackedWidget和QLabel的颜色)
        label.setStyleSheet('background: rgb(%d, %d, %d);margin: 50px;' % (
        ...
        item = QListWidgetItem(QIcon('Data/0%d.ico' % randint(1, 8)), '批量邀请达人', self.listWidget)
        item.setSizeHint(QSize(16777215, 60))
        item.setTextAlignment(Qt.AlignCenter)

        label = QLabel('批量邀请页面', self)
        label.setAlignment(Qt.AlignCenter)
        # 设置label的背景颜色(这里随机)
        # 这里加了一个margin边距(方便区分QStackedWidget和QLabel的颜色)
        label.setStyleSheet('background: rgb(%d, %d, %d);margin: 50px;' % (
